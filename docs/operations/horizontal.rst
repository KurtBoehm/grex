...
Reductions over all lanes of a vector or mask, producing a scalar result.
...
.. cpp:function:: template<Vectorizable T, std::size_t N> \
   ...
   - **128-bit**:
     ...
     - **8-bit integers**: ``sad_epu8`` plus masking via :ref:`blend_zero <operations-blend-zero-static>` with compile-time selectors.
...
.. cpp:function:: template<Vectorizable T, std::size_t N> \
   ...
   .. math::
      ...
      \operatorname{horizontal\_min}(v) = \min_{0 \le i < N} v_i, \qquad
      \operatorname{horizontal\_max}(v) = \max_{0 \le i < N} v_i.
