...
constexpr T bitfield_insert(T dest, T src) {
  ...
  return (dest & ~mask) | ((src & field_mask) << tOffset);
}

// Wrapper that either constant-folds or emits `bfi` on AArch64.
template<std::size_t tOffset, std::size_t tWidth, std::unsigned_integral T>
GREX_ALWAYS_INLINE inline T bfi(T dest, T src) {
  static_assert(tWidth > 0, "WIDTH must be > 0");
  static_assert(tOffset + tWidth <= sizeof(T) * CHAR_BIT, "Field out of range");
  ...
  if (!__builtin_constant_p(dest) && !__builtin_constant_p(src)) {
    if constexpr (sizeof(T) == 8) {
      asm("bfi %0, %1, %2, %3" : "+r"(dest) : "r"(src), "i"(tOffset), "i"(tWidth));
      return dest;
    } else if constexpr (sizeof(T) == 4) {
      asm("bfi %w0, %w1, %2, %3" : "+r"(dest) : "r"(src), "i"(tOffset), "i"(tWidth));
      return dest;
    }
  }

  // Fallback: generic implementation.
  return bitfield_insert<tOffset, tWidth>(dest, src);
}
