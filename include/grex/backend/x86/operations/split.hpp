...
#define GREX_SPLIT_LOWER(KIND, BITS, SIZE, HALF, BITPREFIX, REGISTERBITS, WORKKIND) \
  GREX_SPLIT_WRAP(KIND, BITS, SIZE, HALF, \
                  GREX_CAT(BITPREFIX##_cast, GREX_SIR_SUFFIX(WORKKIND, BITS, REGISTERBITS), _, \
                           GREX_SIR_SUFFIX(WORKKIND, BITS, GREX_DIVIDE(REGISTERBITS, 2)))(v.r))
...
#define GREX_SPLIT_256_1(KIND, BITS, SIZE, HALF, BITPREFIX, REGISTERBITS, WORKKIND) \
  GREX_SPLIT_WRAP(KIND, BITS, SIZE, HALF, \
                  GREX_CAT(_mm256_extract, GREX_SPLIT_LETTER(WORKKIND), 128_, \
                           GREX_SI_SUFFIX(WORKKIND, BITS, 256))(v.r, 1))
// 512 bit
#define GREX_SPLIT_512_0 GREX_SPLIT_LOWER
#define GREX_SPLIT_512_1_f16(...) \
  _mm256_castpd_si256(_mm512_extractf64x4_pd(_mm512_castsi512_pd(v.r), 1))
...
#define GREX_SPLIT_ALL(REGISTERBITS, BITPREFIX) \
  GREX_FOREACH_TYPE_EXT(GREX_SPLIT, REGISTERBITS, BITPREFIX, REGISTERBITS)
...
#define GREX_SPLIT_f64x2_1(KIND, BITS, SIZE) \
  _mm_castsi128_ps(_mm_unpackhi_epi64(_mm_castps_si128(v.registr()), _mm_castps_si128(v.registr())))
#define GREX_SPLIT_i64x2_0(KIND, BITS, SIZE) v.registr()
#define GREX_SPLIT_i64x2_1(KIND, BITS, SIZE) _mm_unpackhi_epi64(v.registr(), v.registr())
...
GREX_SPLIT_SUB_ALL(f, 16, 8, GREX_SPLIT_i64x2)
...
// 32×2
GREX_SPLIT_SUB_ALL(f, 16, 4, GREX_SPLIT_i32x2)
