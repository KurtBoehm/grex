...
#define GREX_HMINMAX_f32x4(OP, ...) \
  ...
  /* [op(v0, v2, v1, v3), -, -, -][0] */ \
  return _mm_cvtss_f32(_mm_##OP##_ss(pairs, shuffled));
...
#define GREX_HMINMAX_i32x4(OP, KIND, BITS, ...) \
  ...
  const auto pairs = OP(v, {.r = unpackhi}); \
  /* [op(v1, v3), -, -, -] */ \
  const __m128i shuffled = _mm_shuffle_epi32(pairs.r, 0x01); \
  /* [op(v0, v2, v1, v3), -, -, -][0] */ \
  return KIND##BITS(_mm_cvtsi128_si32(OP(pairs, {.r = shuffled}).r));
