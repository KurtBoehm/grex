...
// AVX/AVX2 provide more conversion intrinsics than baseline x86-64, which simplifies
// many of the integer and floating-point conversions.

namespace grex::backend {
// Increasing integer size

// Double integer size (sign- or zero-extend).
...
// Decreasing integer size: for narrowing, signed/unsigned behave the same.
...
#else
// u64→f64: convert upper/lower 32 bits separately via exponent manipulation,
// then add the two contributions. This is exact for all u64.
...
// i64 → f64: extract and convert each lane scalarly.
...
// u32 → f64: straightforward because the conversion is exact.
#define GREX_CVT_IMPL_f64_u32_4(...) \
  /* convert to 64 bits to serve as mantissa */ \
  const __m256i vi64 = _mm256_cvtepu32_epi64(v.r); \
  /* 2^52 as an exponent */ \
  const __m256i exponent = _mm256_set1_epi64x(0x4330000000000000); \
  /* attach exponent so mantissa holds the original integer plus hidden bit 2^52 */ \
  const __m256i vf64 = _mm256_or_si256(vi64, exponent); \
  /* subtract 2^52 to cancel the hidden bit */ \
...
#define GREX_CVT_IMPL_f32_u32_8(...) \
  /* replace upper 16 bits with exponent 23; hidden bit adds 2^23 */ \
  const __m256i lf32 = _mm256_blend_epi16(v.r, _mm256_set1_epi32(0x4B000000), 0b10101010); \
  /* shift away lower 16 bits, leaving upper 16 bits */ \
  const __m256i hu16 = _mm256_srli_epi32(v.r, 16); \
  /* set exponent for upper 16 bits to 39 = 23 + 16; hidden bit adds 2^39 */ \
...
#define GREX_CVT_IMPL_u64_f64_4(...) \
  /* v - 2^63 as f64x4; map u64 → i64 domain */ \
  const __m256d voff = \
...
#define GREX_CVT_IMPL_u64_f32_4(...) \
  /* v - 2^63 as f32x4; map u64 → i64 domain */ \
...
#define GREX_CVT_IMPL_u32_f32_8(...) \
  ...
  /* v - 2^31 as f32x8; map u32 → i32 domain */ \
