...
// On x86-64-v2+, use generic EPUI helper (pmovzx/pmovsx) for all widening casts.
...
// [a0, a1, ...] (SRCBITS) → [a0, 0, a1, 0, ...] (DSTBITS = 2*SRCBITS)
...
// Recursive case for larger increases: widen in two steps
//  1) cast to half-sized integer (Half)
//  2) cast from Half to destination type
...
#else
// i8→i16: unpacklo with itself, then srai to extend sign from the high byte
...
// i16→i32: duplicate each 16-bit element and arithmetic-shift by 16 to sign-extend
#define GREX_CVT_IMPL_i32_i16_4(...) \
  return {.r = _mm_srai_epi32(_mm_unpacklo_epi16(v.registr(), v.registr()), 16)};

// i32→i64: create sign mask per element via srai, then unpacklo to interleave value/sign
#define GREX_CVT_IMPL_i64_i32_2(...) \
  return {.r = _mm_unpacklo_epi32(v.registr(), _mm_srai_epi32(v.registr(), 31))};

// Unsigned doubling: use generic Ux2 logic (zero-extend via unpacklo with zero)
...
// Larger integer widening fall back to recursive half-step widening
...
// Same logic for signed/unsigned: simply drop high bits.

#if GREX_X86_64_LEVEL >= 4
// Use truncation intrinsics where possible.
...
// Use pshufb-based element selection for truncation.

// u16x8 → u8x8: take every second byte of the low 8 elements
...
// u32x4 → u16x4: select low 2 bytes from each 32-bit lane
...
// 2-lane variant reuses same shuffle
#define GREX_CVT_IMPL_u16_u32_2 GREX_CVT_IMPL_u16_u32_4

// u64x2 → u32x2: keep low 32 bits from each 64-bit lane using insertps trick
...
// u32x4 → u8x4: select lowest byte of each 32-bit element
...
// u64x2 → u16x2: keep low 16 bits from each 64-bit element
...
// u64x2 → u8x2: keep single lowest byte from each 64-bit element
...
#else
// Level 1: emulate truncation using older shuffles and pack instructions.

// u16→u8 on level 1: mask out upper 8 bits and use a saturated cast as truncation helper
#define GREX_CVT_IMPL_u8_u16_8(...) \
  const auto r = _mm_packus_epi16(_mm_and_si128(v.r, _mm_set1_epi16(0xFF)), _mm_setzero_si128()); \
  return SubVector<u8, 8, 16>{r};

// super-native variant: u16x16 → u8x16 in two 128-bit halves
...
// u32x4 → u16x4: drop upper half of each 32-bit lane via 16-bit shuffles
...
// u32x2 → u16x2: same pattern restricted to low 2 lanes
#define GREX_CVT_IMPL_u16_u32_2(...) \
  /* lo = [u16(v[0]), u16(v[1]), -, ...] */ \
  return SubVector<u16, 2, 8>{_mm_shufflelo_epi16(v.registr(), 0b1000)};

// u64x2 → u32x2: cast via shufps, keep low 32 bits of each 64-bit lane
#define GREX_CVT_IMPL_u32_u64_2(...) \
  /* [u32(v[0]), u32(v[1]), 0, 0] */ \
...
// super-native variants at level 1, handling larger vectors by stitching halves
...
// sub-native variant: u64x2 → u16x2 via u32x2 intermediate shuffle
#define GREX_CVT_IMPL_u16_u64_2(...) \
...
// u64x2 → u8x2 via staged narrowing through 32- and 16-bit lanes
#define GREX_CVT_IMPL_u8_u64_2(...) \
  ...
  /* final pack to u8x16, only first two bytes used */ \
  return SubVector<u8, 2, 16>{_mm_packus_epi16(vu16, _mm_setzero_si128())};
#endif

#if GREX_X86_64_LEVEL < 3
// 256-bit super-native variant for 32→64 bit widening across two 128-bit halves
#define GREX_CVT_IMPL_i32_i64_4(DSTKIND, ...) \
  /* take low 32 bits from lower/upper halves and merge into single 128-bit */ \
  /* [u32(v[0]), u32(v[1]), 0, 0] */ \
  const __m128 sh = \
...
//////////////////////////////
// The SMALLI2F helpers handle small integer types via 32-bit intermediates.

//                   //
// Conversion to f64 //
//                   //
#if GREX_X86_64_LEVEL >= 4
// Direct packed cvt intrinsics for 64-bit and 32-bit integer sources
...
#else
// u64→f64: split into low/high 32-bit parts; encode them as doubles via exponent
// fiddling and then sum them. This is exact for all u64.
#define GREX_CVT_IMPL_f64_u64_2(...) \
  ...
  /* add exponent 52: interpret as double with integer in mantissa; \
   * hidden bit contributes +2^52 */ \
  ...
  /* set exponent to 84 = 52 + 32; hidden bit contributes +2^84 */ \
  const __m128d hf64 = _mm_castsi128_pd(_mm_or_si128(hu32, _mm_set1_epi64x(0x4530000000000000))); \
  /* subtract 2^84 * (1 + 2^-32) = 2^84 + 2^52 to cancel hidden bits of both parts */ \
  const __m128d hsub = _mm_sub_pd(hf64, _mm_castsi128_pd(_mm_set1_epi64x(0x4530000000100000))); \
  /* add both contributions to get exact u64 as f64 */ \
  return {.r = _mm_add_pd(lf64, hsub)};

// i64 → f64: convert lanes via scalar cvtsi2sd and re-pack
...
// u32 → f64: pack into mantissa, set exponent to 52, then subtract hidden bit.
// Exact for all u32.
#define GREX_CVT_IMPL_f64_u32_2(...) \
  ...
  /* exponent 52 encoded in IEEE754 f64 */ \
...
// i32→f64 uses standard cvt, small ints via 32-bit intermediate
...
#else
// i64 → f32: scalar convert each lane and repack to a 2-lane subvector
...
// u64 → f32: round v/2 to i64, convert to f32, then multiply by 2.
// Rounding-to-even via (v>>1) | (v&1) ensures sign bit cleared.
...
#elif GREX_X86_64_LEVEL >= 2
// x86-64-v2: split into low/high 16 bits, encode via exponents and recombine.
...
#else
// Same idea as above but without pblendw: explicit masking/OR.
#define GREX_CVT_IMPL_f32_u32_4(...) \
...
// i32→f32 and small integers use intrinsics and conversions to 32 bits if necessary.
...
//                     //
#if GREX_X86_64_LEVEL >= 4
// Direct packed cvtt intrinsics for 64/32-bit integer destinations
...
#else
// f64x2 → i64x2 via scalar truncating conversions of both lanes
...
// f64x2 → u64x2: offset by 2^63 to map u64 domain into i64, convert, then select
// between plain and offset result based on sign/overflow.
...
// f64x2 → u32x2: same offset trick at 32-bit width
...
// Smaller integer destinations via i32
...
#else
// f32x2 → i64x2: convert each lane using scalar cvttss2si and repack
...
#if GREX_X86_64_LEVEL == 1
// Level-1 super-native variants: operate on larger logical vectors via 128-bit pieces
...
#if GREX_X86_64_LEVEL < 3
// 256-bit super-native widening from 32→64 bit types using two 128-bit lanes
